...
# Initiate a new PIL based display tool from display_utils
Backend.PIL()
...
# overall index.
def ssim(window0, window1, W, alpha, beta, gamma, general_form=True, *args):
...
# Mean SSIM
# This function calculates SSIM for each corresponding sub window of the two images
...
def mssim(img0, img1, alpha = 1, beta = 1, gamma = 1):
    ...
    # Initiate an empty list of SSIM values
    combined_ssim = []

    # A mini function that takes two single channel images to first compute SSIMs for
    # every corresponding sub window of them and then ad these values to aforementioned list.
    ...
    # Split the channels and process them seperately if the images are polychrome
    if len(img0.shape) == 3:
